...
void  CGuiMiniTool::AddButtonIcon(UINT uIDIcon,UINT Cmd,CMenu* pMenu,CString m_szCaption,CString m_ToolTip)
{
	HICON hIcon=AfxGetApp() ->LoadIcon(uIDIcon); 
	if(hIcon==NULL)		return ; 
	
	m_arrButtons.SetAtGrow(nNumItems, new CArrButton
			(this,hIcon,Cmd,&m_cfont,m_szCaption,pMenu,m_ToolTip));
...
void CGuiMiniTool::EnableButton(UINT unID, BOOL bEnable)
{
	GetButton(unID)->EnableWindow(bEnable);
}

void CGuiMiniTool::SetButtonState(UINT unID, BOOL bPressed)
{
	GetButton(unID)->SetState(bPressed);
}

BOOL CGuiMiniTool::GetButtonEnabled(UINT unID)
{
	return GetButton(unID)->IsWindowEnabled();
}

BOOL CGuiMiniTool::GetButtonState(UINT unID)
{
	return (GetButton(unID)->GetState()&0x0004);
}

void CGuiMiniTool::SetButtonToolTip(UINT unID, LPCTSTR str)
{
	GetButton(unID)->SetToolTip(str);
}

void CGuiMiniTool::SetButtonCaption(UINT unID, LPCTSTR str)
{
	GetButton(unID)->SetCaption(str);
	GetButton(unID)->Invalidate();
	GetButton(unID)->UpdateWindow();
}
