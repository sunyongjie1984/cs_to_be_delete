...
BOOL IOCPS::AssociateIncomingClientWithContext(SOCKET clientSocket)
{
	...
	if (m_bShutDown||!m_bAcceptConnections||clientSocket==INVALID_SOCKET)
		return FALSE;


	//
	// Close connecttion if we have reached the maximum nr of connections... 	
	// 	
...
/*
void CSimpleServer::AddJob(PACKETWORK *pPacketWork)
{
m_WorkList.AddHead((void*)pPacketWork);
}

void CSimpleServer::FreeWorkList()
{
while(!m_WorkList.IsEmpty())
{
PACKETWORK *pPacketWork=(PACKETWORK *)m_WorkList.RemoveHead();
if(pPacketWork)
{
delete pPacketWork;
}
...
void IOCPS::OnWrite(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{	
		...
		while(pOverlapBuff!=NULL)
		{
			...
			/*
			* Comments about bugg in OnWrite() added 051227.. 
			* 
			...
			* that sometimes the client specific data (eg ClientContext) does not removed/relesed 
			...
			*/  
			...
			if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) 
			{
				...
				DisconnectClient(pContext);
				pOverlapBuff=NULL;
				/* "the bug fix 051227": Check if we need to go out of some more pendling IO.
				...
				TRACE(">OnWrite(%x)\r\n",pContext);
				ReleaseClientContext(pContext); 	// pContext may not exist efter this call 	
...
void IOCPS::OnWriteCompleted(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	if(pContext!=NULL)
	{
		...
		//
		// We have finished a Complete LOOP, if no pendling IO is in the competetion port remove the client. 
...
BOOL IOCPS::ASend(ClientContext *pContext,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		//
		// Important!! Notifyes that the socket and the structure  
		// pContext have an Pendling IO operation ant should not be deleted.
		// This is nessesary to avoid Access violation. 
		//
		EnterIOLoop(pContext); 
	...
	{	
		...
		{		
			...
			if(m_bOneIPPerConnection)
			{
				...
				//
				//  Remove the IP adress from list.. 
				//	
...
* OBS! this function should not be called if there is any pendling operations. 
*
*
*/
...
#if defined SIMPLESECURITY	
/*
inline void IOCPS::DisconnectIfIPExist(SOCKET &Socket)
{
...
m_OneIPPerConnectionLock.Lock();
POSITION pos=m_OneIPPerConnectionList.Find(sIP);
if(pos!=NULL)
{
//
// Disconnect. 
...
//
if(	Socket != INVALID_SOCKET)
m_OneIPPerConnectionList.AddHead(sIP);
m_OneIPPerConnectionLock.Unlock();	
...
*	Adress. Add the CLient to the list othervise.  
*  
...
/*
void IOCPS::DisconnectIfBanned(SOCKET &Socket)
{
CString sIP="";
//
//  Get the incoming connection IPadress
...
if(!m_BanIPList.IsEmpty())
{
POSITION pos=m_BanIPList.Find(sIP);
if(pos!=NULL)
{
//
// Disconnect. 
