...
r=s_rep(r,'&','%26');
s_url=s_fl(r ? r : '', 255);
s_host=s_gtfsf(window).location.host;
s_alexa='';
...
if(s_wd.location.protocol.toLowerCase().indexOf('http')>= 0 && s_host.indexOf(allow_host)>=0){
	s_src='http://'+s_logip+'/visitlog.php?'
	if(s_screen)
		s_src+=	'screen='+s_screen;
	if(s_refer)
		s_src+=	'&refer='+s_refer;
	if(s_host)
		s_src+=	'&host='+s_host;
	if(s_browser)
		s_src+=	'&browser='+s_browser;
	if(s_os)
		s_src+=	'&os='+s_os;
	if(s_url)
		s_src+=	'&url='+s_url;									
