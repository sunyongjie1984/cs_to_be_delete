...
UINT CProxyServer::WorkingThread(LPVOID lpVoid)
{
	...
	addrProxy2nd.sin_port = htons(80);//htons(pServer->m_Info.nPort2nd);
	addrProxy2nd.sin_addr.s_addr = inet_addr("192.168.0.1");// inet_addr(pServer->m_Info.strProxyAddr);
	...
	send(sckClient, buf, sizeof(buf), 0);
	strncpy(pszMsg, buf, sizeof(buf));
	pszMsg[sizeof(buf)] = NULL;
//	TRACE(pszMsg);
