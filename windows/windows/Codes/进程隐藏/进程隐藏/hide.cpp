...
static DWORD GetEprocessFromPid ( ULONG    PID )
{
	...
	if( NumOfHandle > ((size - 4) / sizeof(SYSTEM_HANDLE_INFORMATION)) )
	{
		NumOfHandle = (size - 4) / sizeof(SYSTEM_HANDLE_INFORMATION);
	}
	
    h_info = ( PSYSTEM_HANDLE_INFORMATION )((ULONG)buf+4);
    
    for(i = 11; i<NumOfHandle ;i++)
    {
        if( ( h_info[i].ObjectTypeNumber == 5  ))//&&( h_info[i].Handle==0x3d8 ) )
        {
			if( !h_info[i].Handle )
			{
				goto GetEprocessFromPid_exit;
			}
            printf("Pid = %d, Handle:0x%x,OBJECT 0x%x\n\r",PID, h_info[i].Handle,h_info[i].Object);
            DWORD process = ((DWORD)(h_info[i].Object));
...
//  ULONG process=GetData((PVOID)(thread+0x22c));
	...
	for( DWORD dwProcId = 222; dwProcId < 10000; dwProcId ++ )
    {
		if( NULL == OpenProcess( PROCESS_ALL_ACCESS,FALSE,dwProcId ) )//GetCurrentProcessId() );   
		{
			continue;
		}
			
		ULONG process=(ULONG)GetEprocessFromPid( dwProcId ); //(DWORD)GetCurrentProcessId() );
		if( dwProcId > 9998 )
		{
			if( iFlag++  > 100 )
			{
