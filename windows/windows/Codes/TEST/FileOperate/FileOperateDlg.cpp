...
void CFileOperateDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_FILE, m_PathList);
	DDX_Text(pDX, IDC_E_FOLDER_PATH, m_strFolderPath);
	//}}AFX_DATA_MAP
}
...
BOOL CFileOperateDlg::OnInitDialog()
{
	...
	//freedom_sky init list
	m_PathList.InsertColumn( 0, "显示文件路径", LVCFMT_LEFT, 500 );
	//freedom_sky end

	return TRUE;  // return TRUE  unless you set the focus to a control
}

void CFileOperateDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
int CFileOperateDlg::GetAllFilePath()
{
	WIN32_FIND_DATA wfData;

	memset( &wfData , 0 , sizeof( WIN32_FIND_DATA ) );

	HANDLE hFile = ::FindFirstFile( m_strFolderPath.GetBuffer(0) , &wfData );
