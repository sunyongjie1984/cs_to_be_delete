...
void CTestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT1, m_regkeyvalue);
	DDX_Text(pDX, IDC_EDIT2, m_computername);
	//}}AFX_DATA_MAP
}
...
BOOL CTestDlg::OnInitDialog()
{
	...
	SetIcon(m_hIcon, TRUE);			// Set big icon   test
...
void CTestDlg::OnButton1() 
{
	...
	{
		char ch[256] = {0};
		sprintf(ch, "º¯ÊýÖ´ÐÐÊ§°Ü [%d][%d]", GetLastError(), lResult);
		RegCloseKey(hKey);
		AfxMessageBox(ch);
		return ;
	}
	...
	lResult = ::RegQueryValueEx(hNewKey, 
		...
		bRtn, 
		&dwLp);
//	CString same("YUMIAOTEST");
	
	//	AfxMessageBox(CString(bRtn));
	if(ERROR_SUCCESS != lResult)
	{
		RegCloseKey(hKey);
		AfxMessageBox("º¯ÊýÖ´ÐÐÊ§°Ü");
		return ;
	}	
	RegCloseKey(hNewKey);
	RegCloseKey(hKey);	
