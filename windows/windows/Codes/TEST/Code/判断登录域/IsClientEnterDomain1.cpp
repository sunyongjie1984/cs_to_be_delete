...
int CIsClientEnterDomain::GetGroupInfo(LPSTR strServerName, UINT *uType)
{
	...
	HMODULE hLoad = ::LoadLibrary("Netapi32.dll");//获取dll模块句柄
	if(!hLoad)
	{
...
int CIsClientEnterDomain::Is_Win2000_DomainController()
{
	...
	P_VerifyVersionInfo pVerifyVersionInfo = (P_VerifyVersionInfo)::GetProcAddress(hLoad, "VerifyVersionInfoA");//加A是ASIIC类型
	                                                                                                            //加W是宽字符
	...
	BOOL bRet = pVerifyVersionInfo(&osvi, //
