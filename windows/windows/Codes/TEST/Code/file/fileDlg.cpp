...
void CFileDlg::OnRead() 
{
	CString  strname = "C:\\Documents anrd ÎÄµµ.doc";
	FILEINFO ffileinfo;
	CFile    readfile;
	readfile.Open(strname, CFile::modeRead|CFile::typeBinary);
	ffileinfo.filelength = readfile.GetLength();
	strcpy(ffileinfo.filename,readfile.GetFileName());
	readfile.Seek(0, CFile::begin);
	char buff[1024] = {0};
	while (true)
	{
		int num=readfile.Read(buff, 1024);
		if(num==0)
		{
			break;
		}
