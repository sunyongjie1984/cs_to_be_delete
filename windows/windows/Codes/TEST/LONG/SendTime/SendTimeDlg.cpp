...
void CSendTimeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_MSG, m_nTime);
	//}}AFX_DATA_MAP
}
...
void CSendTimeDlg::OnSend() 
{
	UpdateData();

	char szIPAddress[1024] = {0};

	strcpy( szIPAddress, m_strIP.GetBuffer(0) );

	WSADATA wsaData;

	int iResualt = WSAStartup(MAKEWORD(2, 2), &wsaData);

	SOCKET  socketA = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	...
	service.sin_addr.S_un.S_addr = inet_addr( szIPAddress );

	if(connect(socketA, (SOCKADDR*)&service, sizeof(service)) == SOCKET_ERROR)
	{
		WSACleanup();
	}
	...
	sprintf( szSendbuf, "%d", m_nTime );

	send(socketA, szSendbuf, strlen(szSendbuf), 0);

	recv( socketA, szSendbuf, 32, 0 );

	CString strTimeHint;
	strTimeHint.LoadString(IDS_TIME_HINT);

	::MessageBox( NULL, szSendbuf, strTimeHint, MB_OK );
}
