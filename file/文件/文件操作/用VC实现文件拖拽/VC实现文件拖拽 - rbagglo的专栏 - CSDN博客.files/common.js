...
$(document).ready(function() {
    ...
    $("#buttonSearch").click(function() {
        ...
        if (searchText) {
            var searchUri = "http://so.csdn.net/BlogSearchResult.aspx?q=" + encodeURIComponent(searchText);
            if (searchScope != "all")
                searchUri = searchUri + " username:" + encodeURIComponent(searchScope);
    ...
    var guest = !username || /^guest$/i.test(username);
    if (guest) {
		...
		$("li:has(#a_welcome,#a_exit,#a_myblog,#a_configure,#a_postedit,#a_postlist,#a_bloghome)").show();
        $("#a_welcome")
			.html("欢迎&nbsp;" + username + "!")
