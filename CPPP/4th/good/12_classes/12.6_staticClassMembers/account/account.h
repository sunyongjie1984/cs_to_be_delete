#ifndef ACCOUNT_SUNYONGJIE
#define ACCOUNT_SUNYONGJIE
#include <string>
#include <iostream>
using std::string;
using std::istream;
...
private:
    ...
    string owner;
    double amount;
	// dont treat static data members as members,
	// it is almost a golbal data
	...
	// defination, out side of header, only declare in class
	// the reason for that, I think, anything in C++(global function, variable
	// can only be defined once, except one
	// exception, the defination of class, take this program for example,
	// main.cpp, account.cpp all #include the account.h, and the defination
	// of class Account(so do the data members, function members has two
    // copies, but with on compile errors, so when
	// it comes to the static data member is not a class defination, that is
	// why static data member can not be defined in header file but
	// a class Account can. The rule is that, same class can be defined in
	// multi files as long as the definations are excatly the same.
	
	// remember dont treat static data member as normal data member,
	// static function member can define as normal function member
