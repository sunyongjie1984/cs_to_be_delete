...
void test01()
{
    ...
    {
        int const i = GetFileSize(hFile, NULL);
        if( 100 < i )
        {
...
void delete_file_syj(char const * p)
{
    std::cout << p << std::endl;
    ::DeleteFile(p);
    std::cout << "delete_file_syj" << std::endl;
}

void close_handle_syj(HANDLE h)
{
    ::CloseHandle(h);
    std::cout << "close_handle_syj" << std::endl;
}
...
void test02_1()
{
    // delete_file_syj要与::DeleteFile函数原型相同
    // boost::shared_ptr<char const> spFilePath( "c:\\test.dat", &::DeleteFile );
    boost::shared_ptr<char const> spFilePath( "c:\\test.dat", &delete_file_syj );
    HANDLE hFile( ::CreateFile( spFilePath.get(), GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, 0, NULL ) );

    // close_handle_syj要与::CloseHandle函数原型相同
    // boost::shared_ptr<void> spFile( hFile, &::CloseHandle );
    boost::shared_ptr<void> spFile( hFile, &::close_handle_syj );
    if( INVALID_HANDLE_VALUE != spFile.get() )
    {
        DWORD dwFileSize = 0;
        ::GetFileSize( spFile.get(), &dwFileSize );
    }
} // 不知道是因为我没有把boost的源码加进来，还是什么其它的原因，按F11不能进test_syj_1，但是设置断点能走进去
  // 然后再call stack里能看到在boost中的函数stack
...
void test03_01()
{
    // char const *类型作为参数实例化模版，
    // char const *类型"c:\\test.dat"为构造函数参数一
    // 函数地址类型    &delete_file_syj为构造函数参数二，delete_file_syj接受是一个char const *对象作形参 
    //am::mate<char const *> lpszFilePath( "c:\\test.dat", &::DeleteFile );
    am::mate<char const *> lpszFilePath( "c:\\test.dat", &delete_file_syj );
    HANDLE h( ::CreateFile( lpszFilePath, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, 0, NULL ) );
    // HANDLE类型作为参数实例化模版，
    // HANDLE类型h为构造函数参数一
    // 函数地址类型    &close_handle_syj为构造函数参数二，close_handle_syj接受是一个HANDLE对象作形参 
    //am::mate<HANDLE> hFile( h, &::CloseHandle,
    //    (HANDLE)INVALID_HANDLE_VALUE  != lambda::_1 || ( (HANDLE)123 <= lambda::_1 && ! true ) );
    am::mate<HANDLE> hFile( h, &::close_handle_syj,
        (HANDLE)INVALID_HANDLE_VALUE  != lambda::_1 || ( (HANDLE)123 <= lambda::_1 && ! true ) );
    if( INVALID_HANDLE_VALUE != hFile )
    {
        DWORD dwFileSize = 0;
...
int prefix(int n) 
{
    std::cout << "entering fuction prefix" << std::endl;
    return n + 100;
}
void suffix(int n)
{ 
    std::cout << "entering fuction suffix" << std::endl;
    n -= 100;
}
...
{
    //am::mate<int> nNumber(prefix(123), &suffix);
    am::mate<int> nNumber(223, &suffix);
...
void test05_01(HWND hwnd)
{   // c++参数传递很可能是从右向左，因为下面明显是先调用MyReleaseDC的构造函数 
    am::mate<HDC> hdc( ::GetWindowDC( hwnd ), MyReleaseDC( hwnd ) );
