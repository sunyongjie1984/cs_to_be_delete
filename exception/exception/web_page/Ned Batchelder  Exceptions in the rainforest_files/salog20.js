...
HubspotLog.prototype = {
	Init : function() {
		...
		this.opt_out = this.ReadCookie("opt_out") == "yes";
	...
	Log : function(){
		...
		try
		{
			...
			if (window["hs_page_id"]) {
				this.pageId = window["hs_page_id"];
				this.pageId = encodeURIComponent(this.pageId)
			}

            if (window["hs_content_type"]) {
                this.contentType = window["hs_content_type"];
                this.contentType = encodeURIComponent(this.contentType);
            }
	...
	GenerateQueryString20 : function() {
		...
		if (this.pageId) {
			query += "&pi=" + this.pageId;
		}

        if (this.contentType) {
            query += "&ct=" + this.contentType;
        }
