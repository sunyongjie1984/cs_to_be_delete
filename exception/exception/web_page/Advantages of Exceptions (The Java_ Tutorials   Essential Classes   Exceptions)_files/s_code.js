...
function s_doPlugins(s) {
	...
	s.baseURL=decodeURI(location.protocol+"//"+location.hostname+location.pathname.replace(/\/index\.htm$/,""));
	else
	s.baseURL=decodeURI(location.protocol+"//"+location.hostname+location.pathname.replace(/\/index\.html$/,""));
	
	if ( location.href.indexOf('eblast_links_redirect') != -1 ){
		s.baseURL = s.getQueryParam('target_url');
	}
	...
	/* track eBlast/Message Unique Users id: lid /
	/ --------------------------- */
	if (s.getQueryParam('lid')){
		s.eVar11 = s.getQueryParam('lid');
	}
		
	/* track eBlast/Message Unique Users id: eid /
	/ --------------------------- */
