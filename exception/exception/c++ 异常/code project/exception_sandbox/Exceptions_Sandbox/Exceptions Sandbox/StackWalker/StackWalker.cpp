...
BOOL StackWalker::ShowCallstack(int skipFramesCount, HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0 && frameNum >= skipFramesCount)
    {
      ...
      { // yes, we have SymGetLineFromAddr64()
        ...
        {
			if ( (m_options & StackWalker::RetrieveGeneralInfo) != 0 ) 
			{
				//Normally there's no need to write this down, it can be seen that the filename is missing from the stack trace itself
